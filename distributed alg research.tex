...
No more than one node will have leader set to true at any point.
\end{lemma}
\begin{proof}

  Any breadth-first search instance, $bfs$ with source equal to a node $n$ where $n$ is not equal to $U_{max}$ will send a search message to a node, $n_1$, that is running a BFS instance, $bfs_1$, whose source is \textgreater $n$ \\
  By definition of the algorithm, $n_1$ will respond with a search message. \\
  Once $n$ receives a search message from $n_1$, it will cease running the $bfs$ instance and will start running $bfs_1$, the BFS instance run by $n_1$. \\
  As a result, $bfs_1$ will never terminate because a node in its tree will never send a done message to its parent. Therefore, $n$ will not set leader to true. \\
\end{proof}

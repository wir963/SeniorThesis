...
A node $v$ will only set leader = true if the breadth-first search (BFS) tree $b$, where sourceID($b$) == id($v$), terminates.

A BFS tree $b$ will only terminate if sourceID($b$) == id($u_{max}$) (Lemma~\ref{BFSTermination})

Only one node, $u_{max}$, will elect itself leader. 

\end{proof}

\begin{lemma}
\label{BFSTermination}
A BFS tree $b$ will only terminate if sourceID($b$) == id($u_{max}$)
\end{lemma}

\begin{proof}
Given a BFS tree $c$ where sourceID($c$) \textless  id($u_{max}$), $c$ will eventually send a search message to a node $x$ running a BFS tree $d$ where sourceID($d$) \textgreater  sourceID($c$).

$w$ is the node in $c$ that will send the search message to $x$.

By definition of the algorithm, node $x$ will respond with a search message where ID == sourceID($d$).

When $w$ receives the search message sent by $x$, it ceases running $c$ and starts running $d$ because sourceID($d$) \textgreater sourceID($c$).

As a result, $w$ will never send a done message to its parent in $c$.

Therefore, $c$ will never terminate.
\end{proof}
